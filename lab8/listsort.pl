...
%------------------------sorting a list of pairs by first element, in descending order---------------------------
% Define a predicate to swap two elements in a pair
swap([A,B], [B,A]).

% Define a predicate to compare pairs based on their first element
compare_pairs([X,_], [Y,_]) :-
    X =< Y.

% Define a predicate to sort a list of pairs by their first element
sort_pairs_by_first([], []).
sort_pairs_by_first([Pair|Pairs], Sorted) :-
    partition(compare_pairs(Pair), Pairs, Less, Greater),
    sort_pairs_by_first(Less, SortedLess),
    sort_pairs_by_first(Greater, SortedGreater),
...
ssort(List, Result) :-
    ...
    count_frequency(L,M),
    sort_pairs_by_first(M,N),   % sorting by length
    iterate_pairs(N,Q),
    reverse_list(Q,T),
